...
**A RAG-based assistant for Bhutan Telecom Limited (BTL)** — it answers customer questions using **retrieval from your own documents** (Excel, JSON, CSV, PDF) and a **local large language model** (Ollama) for natural-language replies grounded in that content. No paid cloud LLM API is required for the default setup.
